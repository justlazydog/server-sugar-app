...
func (*sugar) Create(s model.Sugar) (err error) {
	sqlStr := "insert into sugars (sugar,currency,real_currency,shop_sie,shop_used_sie,account_in,account_out,avg_growth_rate,dat) " +
		"values (?,?,?,?,?,?,?,?,?)"
	_, err = db.MysqlCli.Exec(sqlStr, s.Sugar, s.Currency, s.RealCurrency,
		s.ShopSIE, s.ShopUsedSIE, s.AccountIn, s.AccountOut, s.AvgGrowthRate, s.Dat)
	return
}
...
func (*rewardDetail) Get(userID string) (res model.RewardDetail, err error) {
	...
	sqlStr := "select create_time,today_bal,destroy_hash_rate,growth_rate,balance_hash_rate,invite_hash_rate from reward_detail where " +
		"user_id = ? and create_time >= ? order by id desc limit 1"
	row := db.MysqlCli.QueryRow(sqlStr, userID, t.In(time.Local))
	var createTime string
	err = row.Scan(&createTime, &res.TodayBal, &res.DestroyHashRate, &res.GrowthRate, &res.BalanceHashRate, &res.InviteHashRate)
